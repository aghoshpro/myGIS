function [K1,K2,Z] = DataXtractlonglat(list) 
...
%------- Decide the place ------------------------
XlatMin  =  Uttarakhand(1,2);%Kashmir(1,2);
XlatMax  =  Uttarakhand(2,2);%Kashmir(2,2);
XlongMin =  Uttarakhand(1,1);%72.30;%Kashmir(1,1);
XlongMax =  Uttarakhand(2,1);%Kashmir(2,1);
...
title({['Uttarakhand ' filename];}, 'Interpreter', 'none', ...
       'FontSize',16, 'FontWeight','bold');
axesm('MapProjection','miller','MapLatLimit',[latMin,latMax],...
      'MapLonLimit',[longMin,longMax],'Grid','on',...
      'MeridianLabel','on','ParallelLabel','on','PLabelLocation',2,'PLabelMeridian','prime', ...
...
%caxis([A2min Amax]);
caxis([-50 50]);
T = 'H:\TOTAL_INDIA_SCALED_DATA\Uttarakhand\';
T = strcat(T,filename);
T = strcat(T,'.jpg');
